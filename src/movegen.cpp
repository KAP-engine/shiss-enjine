...
uint64_t compute_king(uint64_t bitboard, uint64_t own_side) { 
   uint64_t bitboard_clip_file_a = bitboard & ~MASK_FILE[file_a];
    ...
    uint64_t spot_1 = bitboard << 8; // top
    ...
    uint64_t spot_5 = bitboard >> 8; // bottom 
    uint64_t spot_6 = bitboard_clip_file_a >> 9; // bottom left
    uint64_t spot_7 = bitboard_clip_file_a >> 1; // left
    uint64_t spot_8 = bitboard_clip_file_a << 7; // top left
